...
(define (populate-rw-rel-with-mat E rw-rel-name mat mat-var-set var-set)
  (define conn (egraph-conn E))
  (define from-clause
    (let ([lst (for/list ([pat mat]
                         [i (in-naturals)])
                (define func (pattern-rel pat))
                (define arity (length (pattern-vars pat)))
                (define rel (hash-ref (egraph-symbols E) (cons func arity)))
                (format "~a f~a" rel i))])
      (string-join lst ", ")))
  ...
  (define where-clause
    (let* ([work (λ (var locs)
                   ...
                   (define wheres (for/list ([loc+ locs+])
                                    (define loc+-string (get-loc-string loc+))
                                    (format "~a = ~a" loc-string loc+-string)))
                   wheres)]
           [_ (displayln mat-var-set)]
